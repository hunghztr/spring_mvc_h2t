...
<main class="ml-[300px] mr-[100px] mt-[30px] p-6 bg-white rounded shadow-md">
    ...
    <form:form modelAttribute="product" method="post" action="/admin/product/update"
        ...
        <div>
            ...
            <form:select path="category.id" cssClass="w-full border border-gray-300 rounded px-4 py-2">
                ...
                <c:forEach var="s" items="${categories}">
                    <option value="${s.id}" <c:if test="${product.category != null && s.id == product.category.id}">selected</c:if>>
                            ${s.name}
                    </option>
                </c:forEach>
            </form:select>
